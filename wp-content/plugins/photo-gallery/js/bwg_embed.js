...
function spider_display_embed(embed_type, file_url, embed_id, attrs) {
  ...
  switch(embed_type) {
    case 'EMBED_OEMBED_YOUTUBE_VIDEO':
      ...
      if(embed_id!=''){
        oembed_youtube_html += ' src="' + 'http://www.youtube.com/embed/'+ embed_id + '?enablejsapi=1&wmode=transparent"';
      }
      for (attr in attrs) {
        if(!(/src/i).test(attr)){
          if(attr != '' && attrs[attr] != ''){
    ...
    case 'EMBED_OEMBED_VIMEO_VIDEO':
      ...
      if(embed_id!=''){
        oembed_vimeo_html += ' src="' + 'http://player.vimeo.com/video/'+embed_id + '?enablejsapi=1"';
      }
      for (attr in attrs) {
        if(!(/src/i).test(attr)){
          if(attr != '' && attrs[attr] != ''){
    ...
    case 'EMBED_OEMBED_INSTAGRAM_IMAGE':
      ...
      oembed_instagram_html += " >";
        ...
        oembed_instagram_html += '<img src="http://instagram.com/p/'+embed_id+'/media/?size=l"'+ 
        ' style=" '+
        'max-width:'+'100%'+" !important"+
        '; max-height:'+'100%'+" !important"+
        '; width:'+'auto'+
    ...
    case 'EMBED_OEMBED_INSTAGRAM_POST':
      ...
      oembed_instagram_html += " >";
        if(embed_id!=''){
        oembed_instagram_html += '<iframe class="inner_instagram_iframe_'+obj_class+'" src="http://instagr.am/p/'+embed_id+'/embed/?enablejsapi=1"'+ 
        ' style="'+
        'max-width:'+'100%'+" !important"+
        '; max-height:'+'100%'+" !important"+
        '; width:'+'100%'+
	...
	case 'EMBED_OEMBED_FACEBOOK_VIDEO': 
      ...
      if(embed_id!=''){
        oembed_facebook_video_html += '<iframe src="http://www.facebook.com/video/embed?video_id='+file_url+'&enablejsapi=1&wmode=transparent"' +
        ' style="'+
        'max-width:'+'100%'+" !important"+
        '; max-height:'+'100%'+" !important"+
        '; width:'+'100%'+
    ...
    case 'EMBED_OEMBED_DAILYMOTION_VIDEO':
      ...
      if(embed_id!=''){
        oembed_dailymotion_html += ' src="' + 'http://www.dailymotion.com/embed/video/'+embed_id + '?api=postMessage"';
      }
      for (attr in attrs) {
        if(!(/src/i).test(attr)){
          if(attr != '' && attrs[attr] != ''){
